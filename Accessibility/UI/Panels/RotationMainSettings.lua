...
local function build(panel)
	...
	local failureThrottle = AceGUI:Create("Slider")
	failureThrottle:SetLabel("Failed-cast throttle")
	failureThrottle:SetSliderValues(0.0, 2.0, 0.05)
	failureThrottle:SetValue(profile.antiSpamWindow or Constants.DEFAULT_FAILURE_THROTTLE_WINDOW)
	failureThrottle:SetCallback("OnValueChanged", function(_, _, value)
		Profile.setFailureThrottle(value)
	end)
	group:AddChild(failureThrottle)
