...
-- from both sides (via Spell.stripRank). Without this an exact match fails and
-- "Aura missing" is true even when the aura is up.
--
...
function Aura.find(unit, name, kind, mineOnly)
	...
	local needle = Spell.stripRank(name):lower()
	...
	for i = 1, Constants.MAX_AURAS do
		...
		if Spell.stripRank(aname):lower() == needle then
