...
typedef void (__cdecl *SecureExecuteFunction)(const char *, const char *, const char *);
...
void secure_executor_run(const char *script, size_t length, const char *ownerName) {
    SecureExecuteFunction execute = (SecureExecuteFunction)SECURE_EXEC;
    char *escaped;
    char *wrapper;
    size_t prefixLength;
    size_t suffixLength;
    size_t escapedLength;
    if (!script || length == 0) { execute(missingScript, ownerName, ownerName); return; }
    if (length > MAX_SCRIPT_LENGTH) { execute(allocationFailure, ownerName, ownerName); return; }
    escaped = lua_bridge_escape_literal(script, length);
    if (!escaped) { execute(invalidScript, ownerName, ownerName); return; }
    ...
    if (!wrapper) {
        ...
        execute(allocationFailure, ownerName, ownerName);
        return;
    }
    ...
    execute(wrapper, ownerName, ownerName);
